...
const StaffScheduling = () => {
  const [staffList, setStaffList] = useState([
    { id: 1, name: 'John Doe', role: 'Receptionist', shifts: ['', '', '', '', '', '', ''] },
    { id: 2, name: 'Jane Smith', role: 'Doctor', shifts: ['', '', '', '', '', '', ''] },
    { id: 3, name: 'Mike Johnson', role: 'Receptionist', shifts: ['', '', '', '', '', '', ''] },
    { id: 4, name: 'Emily Brown', role: 'Doctor', shifts: ['', '', '', '', '', '', ''] },
  ]);
  ...
  const renderShiftOptions = (staffIndex, dayIndex) => {
    ...
    return (
      <select value={staffList[staffIndex].shifts[dayIndex]} onChange={handleShiftChange}>
        <option value="">-</option>
          ...
          {staffList.map((staff, staffIndex) => (
            <tr key={staff.id}>
              ...
              {staff.shifts.map((shift, dayIndex) => (
                <td key={dayIndex}>{renderShiftOptions(staffIndex, dayIndex)}</td>
              ))}
            </tr>
          ))}
        </tbody>
  ...
  return (
    <div className="staff-scheduling-container">
      ...
      {renderScheduleTable()}
      <button className="publish-button" onClick={handlePublishShifts}>Publish Shifts</button>
    </div>
  );
};
